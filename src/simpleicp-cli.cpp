...
int main(int argc, char **argv)
{
  try
  {
    ...
    //std::cout << "Initial Matrix:" << std::endl << X_fix << std::endl << std::endl;
    auto X_mov = ImportXYZFileToMatrix(std::string(result["movable"].as<std::string>()));
    //std::cout << "Initial Matrix:" << std::endl << X_mov << std::endl << std::endl;
    ...
    // Print Outputs
    // std::cout << "Initial Matrix:" << std::endl << X_fix << std::endl << std::endl;
    // X_mov.conservativeResize(X_mov.rows(), X_mov.cols()+1);
    // X_mov.col(X_mov.cols()-1).setOnes();
    // auto X_fixed = (H*(X_mov.transpose())).transpose();
    // std::cout << "Final Matrix:" << std::endl << X_fixed << std::endl;
